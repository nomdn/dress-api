...
const store = getStore("DRESS_CACHE");
async function getCachedIndex(env) {
	try {
		// 正确 await 异步 get

		const cachedID = await store.get("indexID");
		const cachedAuthor = await store.get("indexAuthor");
		const cachedTimestamp = await store.get("indexTimestamp");
	...
	await store.set("indexID", JSON.stringify(indexID));
	await store.set("indexAuthor", JSON.stringify(indexAuthor));
	await store.set("indexTimestamp", timestamp.toString());
